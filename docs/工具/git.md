...
`tail` 用于查看文件末尾内容，也可从指定行开始输出到文件结尾。
...
1. `tail -n 100`：显示文件**最后** 100 行，参数为“数量”。
2. `tail -n +100`：从**第 100 行开始**显示到结尾，参数为“起始行号”，关键在于 `+` 号。
3. `head` 与 `tail` 组合可截取任意行区间，例如查看第 100 至 200 行：

```bash
head -n 200 file.log | tail -n +100
```

其中 `head -n 200` 先取前 200 行，再由 `tail -n +100` 从第 100 行显示，最终得到第 100 至 200 行。只看文件前段时，该方式读到第 200 行即停止，速度较快。

## grep（按条件查询）

`grep` 用于按关键字或正则在文件中检索匹配行，配合参数可定位行号、查看上下文，或只保留部分匹配结果。
...
# 匹配行 + 其后 5 行（After）
grep -A 5 "keyword" file.log

# 匹配行 + 其前 5 行（Before）
grep -B 5 "keyword" file.log

# 匹配行 + 前后各 5 行（Context）
grep -C 5 "keyword" file.log
```

日志排查中，`-n`（定位行号）与 `-C`（查看上下文）的组合最为常用：

```bash
grep -n -C 10 "ERROR" app.log
```

### 保留第几个满足条件的结果

若匹配结果众多，但只关心其中第几个，可结合行号与 `sed` 定位：

```bash
# 先带行号列出全部匹配，再取第 3 个
grep -n "keyword" file.log | sed -n '3p'

# 保留第 3 至第 5 个匹配结果
grep -n "keyword" file.log | sed -n '3,5p'
```

若只需前若干个匹配、找到即停止，可使用 `-m`：

```bash
# 找到 3 个匹配后立即停止扫描，适合大文件
grep -m 3 "keyword" file.log
```

### 使用说明

1. `grep -m N` 会在匹配到 N 条后提前结束，处理大文件时可显著节省时间。
2. `grep -n ... | sed -n 'Np'` 的组合可精确取出第 N 个匹配，适合定位“第几次出现”的场景。

## sed（按行截取与查询）

`sed` 是流式文本处理工具，此处主要用于按行号截取内容，或从其他命令的输出中挑选指定行。
...
其中 `-n` 表示不自动输出每一行，`p` 表示打印指定范围，二者需配合使用。

### 在指定行范围内查询

若既要限定行范围、又要按关键字过滤，可先用 `sed` 截取再 `grep`：

```bash
# 仅在第 100 至 200 行范围内查找 keyword
sed -n '100,200p' file.log | grep "keyword"
```

### 使用说明

1. `sed -n 'n,mp'` 适合精确的行区间截取，是查看指定行最简洁的方式。
2. 配合 `grep -n` 的输出，`sed -n 'Np'` 可从众多匹配中取出第 N 个结果。
